...
public interface CORReceiptListingRepository extends JpaRepository<CORReceiptListing, Long> {
	...
	@Query( 
			value = "select * from vw_daily_receipt_listing where date(receipt_date) = date(:receipt_date) and cash_office_id = :cash_office_id and branch_code = :branch_code and cashier_code = :cashier_code" , 
					nativeQuery = true )
	public List<CORReceiptListing> getReceiptListing(@Param("receipt_date") String receipt_date,
